...
function MedianFilter() {
    ...
    this.convertImage = function (imageData, w,h) {
        ...
        */
        ...
        if (false) { // if (true) : you'll get an equalized image:
            for (var i=0; i<data.length; i+=2) {
                data[i]=rarr[data[i++]]<<2;
                data[i]=garr[data[i++]]<<1;
                data[i]=barr[data[i]]<<2;
            }
            return imageData;
        }
        ...
        if (this.mode==this.FAST) {
            if (this.maskWidth>3 && this.maskHeight>>3) {
                var i=this.maskWidth>>1, j=this.maskHeight>>1;
                // mask.push([i+1,j-1,3]);
                mask.push([i+1,j,3]);
                // mask.push([i+1,j+1,3]);
            }
        ...
        for (var i=0; i<mask.length; ) mask[i++][0]*=4;
    ...
    this.getRowRepeat = function(data,y,w,h,padding) {
        ...
        for (var j=(padding+0)<<2, k=j-4; k>-1; ) {
            var tmp=row.subarray(j,j+3);
            row.set(tmp,k);
            k-=4;
        }

        //  pad to end of array:
        for (var j=(w+padding)*4, k=j-4; j<row.length; ) {
            var tmp=row.subarray(k,k+3);
            row.set(tmp,j);
            j+=4;
        }
        return row;
    ...
    this.getRectangleMask= function( x,y ) {
        ...
        for (var i=0; i<x; arr2[i++]=1){};
        var arr=new Array(y);
        for (var i=0; i<y; arr[i++]=arr2);
        return arr;
    }
